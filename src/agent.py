...
class EconomicAgent(mesa.Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.economic_growth = 0.05 # factor to simulate economic growth from trade
    ...
    def make_trade(self, other):
        if other is not None: 
            trade_amount = (self.wealth + other.wealth) * self.model.economic_growth
            ...
            self.model.total_trade_income += 2 * trade_amount # the total income increase from trade
    ...
    def decide_action(self, other):
        ...
        trade_EU = (other.wealth + self.wealth) * self.model.economic_growth
    ...
    def pay_tax(self):
        # taxes are used to fund services (like policing) which in turn help maintain or increase wealth by reducing crime
        tax_payment = self.wealth * self.model.tax_rate  #wealth tax, could use income tax instead? then the EU calculations are a bit harder
        self.wealth -= tax_payment 
        self.model.total_tax_paid += tax_payment
