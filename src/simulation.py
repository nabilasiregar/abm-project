    ...
    for i in range(iterations):
        for cops in cops_levels:
            ...
            for step in range(num_steps):
                ...
                step_data.update({"Step": step, "iteration": i + 1, "value": cops})
    ...
    for i in range(iterations):
        for length in sentence_lengths:
            ...
            for step in range(num_steps):
                model.step()
                step_data = model.datacollector.get_model_vars_dataframe().iloc[-1].to_dict()
                step_data.update({"Step": step, "iteration": i + 1, "value": length})
                results.append(step_data)
    return pd.DataFrame(results)
    ...
    for i in range(iterations):
        for freq in election_frequencies:
            ...
            for step in range(num_steps):
                model.step()
                step_data = model.datacollector.get_model_vars_dataframe().iloc[-1].to_dict()
                step_data.update({"Step": step, "iteration": i + 1, "value": freq})
                results.append(step_data)
    return pd.DataFrame(results)
...
def main():
    ...
    parser.add_argument("--iterations", type=int, default=30, help="Number of iterations to run each simulation setup")
    ...
    if args.type == 'vary_cops':
        ...
        results = run_varying_sentences([5, 10, 15, 20, 25], 5, args.steps, args.num_agents, args.width, args.height, args.election_frequency, args.interaction_memory, args.iterations)
    elif args.type == 'vary_election_freq':
        results = run_varying_election_frequencies([10, 20, 30, 40, 50], 5, 15, args.steps, args.num_agents, args.width, args.height, args.interaction_memory, args.iterations)
    elif args.type == 'vary_prosperity':
        results = run_varying_prosperity([0.01, 0.05, 1, 1.5, 2, 2.5, 3], 5, 15, args.steps, args.num_agents, args.width, args.height, args.election_frequency, args.interaction_memory, args.iterations)
